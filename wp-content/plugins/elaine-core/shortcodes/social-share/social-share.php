<?php
namespace ElaineCore\CPT\Shortcodes\SocialShare;
...
class SocialShare implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Social Share', 'elaine-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'value'      => array(
								esc_html__( 'List', 'elaine-core' )     => 'list',
								esc_html__( 'Dropdown', 'elaine-core' ) => 'dropdown',
								esc_html__( 'Text', 'elaine-core' )     => 'text'
							)
						),
						array(
							'type'       => 'dropdown',
							'param_name' => 'icon_type',
							'heading'    => esc_html__( 'Icons Type', 'elaine-core' ),
							'value'      => array(
								esc_html__( 'Font Awesome', 'elaine-core' ) => 'font-awesome',
								esc_html__( 'Font Elegant', 'elaine-core' ) => 'font-elegant'
							),
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'title',
							'heading'    => esc_html__( 'Social Share Title', 'elaine-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$params['enable_social_share'] = ( elaine_edge_options()->getOptionValue( 'enable_social_share' ) == 'yes' ) ? true : false;
		
		//Is social share enabled for post type
		$post_type         = str_replace( '-', '_', get_post_type() );
		$params['enabled'] = ( elaine_edge_options()->getOptionValue( 'enable_social_share_on_' . $post_type ) == 'yes' ) ? true : false;
		...
		if ( $params['enable_social_share'] ) {
			if ( $params['enabled'] ) {
				$html .= elaine_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'social-share', '', $params );
			}
		}
	...
	private function getSocialNetworksParams( $params ) {
		...
		foreach ( $this->socialNetworks as $net ) {
			...
			if ( elaine_edge_options()->getOptionValue( 'enable_' . $net . '_share' ) == 'yes' ) {
				...
				$params['custom_icon'] = ( elaine_edge_options()->getOptionValue( $net . '_icon' ) ) ? elaine_edge_options()->getOptionValue( $net . '_icon' ) : '';
				
				$html = elaine_core_get_shortcode_module_template_part( 'templates/parts/network', 'social-share', '', $params );
			}
    ...
    private function getSocialNetworkShareLink($net, $image) {
        switch ($net) {
            ...
            case 'twitter':
                ...
                $twitter_via = (elaine_edge_options()->getOptionValue('twitter_via') !== '') ? ' via ' . elaine_edge_options()->getOptionValue('twitter_via') . ' ' : '';
                if (wp_is_mobile()) {
                    $link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(elaine_edge_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                } else {
                    $link = 'window.open(\'http://twitter.com/home?status=' . urlencode(elaine_edge_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                }
                break;
            case 'google_plus':
            ...
            case 'tumblr':
                ...
                break;
            case 'pinterest':
                $link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . elaine_edge_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
                break;
            case 'vk':
	...
	private function getSocialNetworkText( $net ) {
		switch ( $net ) {
			case 'facebook':
				$text = esc_html__( 'fb', 'elaine-core');
				break;
			case 'twitter':
				$text = esc_html__( 'tw', 'elaine-core');
				break;
			case 'google_plus':
				$text = esc_html__( 'g+', 'elaine-core');
				break;
			case 'linkedin':
				$text = esc_html__( 'lnkd', 'elaine-core');
				break;
			case 'tumblr':
				$text = esc_html__( 'tmb', 'elaine-core');
				break;
			case 'pinterest':
				$text = esc_html__( 'pin', 'elaine-core');
				break;
			case 'vk':
				$text = esc_html__( 'vk', 'elaine-core');
				break;
			default:
				$text = '';
		}
