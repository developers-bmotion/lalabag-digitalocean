...
class Team implements lib\ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ( $x = 1; $x < 6; $x ++ ) {
			$teamIconCollections = elaine_edge_icon_collections()->getCollectionsWithSocialIcons();
			foreach ( $teamIconCollections as $collection_key => $collection ) {
				
				$team_social_icons_array[] = array(
					'type'       => 'dropdown',
					'heading'    => esc_html__( 'Social Icon ', 'elaine-core' ) . $x,
			...
			$team_social_icons_array[] = array(
				'type'       => 'textfield',
				'heading'    => esc_html__( 'Social Icon ', 'elaine-core' ) . $x . esc_html__( ' Link', 'elaine-core' ),
				'param_name' => 'team_social_icon_' . $x . '_link',
				'dependency' => array(
					'element' => 'team_social_icon_pack',
					'value'   => elaine_edge_icon_collections()->getIconCollectionsKeys()
				)
			);
			
			$team_social_icons_array[] = array(
				'type'       => 'dropdown',
				'heading'    => esc_html__( 'Social Icon ', 'elaine-core' ) . $x . esc_html__( ' Target', 'elaine-core' ),
				'param_name' => 'team_social_icon_' . $x . '_target',
				'value'      => array(
					esc_html__( 'Same Window', 'elaine-core' ) => '_self',
					esc_html__( 'New Window', 'elaine-core' )  => '_blank'
				),
		...
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Team', 'elaine-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array_merge(
						array(
							array(
								...
								'value'       => array(
									esc_html__( 'Info Below Image5', 'elaine-core' )    => 'info-below-image',
									esc_html__( 'Info On Image Hover', 'elaine-core' ) => 'info-on-image'
								),
								'save_always' => true
							),
							array(
							...
							array(
								...
								'heading'    => esc_html__( 'Name', 'elaine-core' )
							),
							array(
								'type'        => 'dropdown',
								'param_name'  => 'team_name_tag',
								'heading'     => esc_html__( 'Name Tag', 'elaine-core' ),
							...
							array(
								...
								'heading'    => esc_html__( 'Name Color', 'elaine-core' ),
								'dependency' => array( 'element' => 'team_name', 'not_empty' => true )
							),
							array(
								'type'       => 'textfield',
							...
							array(
								...
								'heading'    => esc_html__( 'Position Color', 'elaine-core' ),
							...
							array(
								...
								'heading'     => esc_html__( 'Social Icon Pack', 'elaine-core' ),
								'value'       => array_merge( array( '' => '' ), elaine_edge_icon_collections()->getIconCollectionsVCExclude( 'linea_icons' ) ),
								'save_always' => true
							),
						),
	...
	public function render( $atts, $content = null ) {
		...
		for ( $x = 1; $x <= $number_of_social_icons; $x ++ ) {
			...
			foreach ( elaine_edge_icon_collections()->iconCollections as $collection_key => $collection ) {
		...
		$html = elaine_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'team', '', $params );
	...
	private function getTeamSocialIcons( $params ) {
		...
		if ( $team_social_icon_pack !== '' ) {
			...
			$icon_pack                    = elaine_edge_icon_collections()->getIconCollection( $team_social_icon_pack );
