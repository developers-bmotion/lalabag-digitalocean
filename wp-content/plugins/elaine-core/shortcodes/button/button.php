<?php
namespace ElaineCore\CPT\Shortcodes\Button;
...
class Button implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Button', 'elaine-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array_merge(
						array(
							...
							array(
								...
								'value'      => array(
									...
									esc_html__( 'Huge', 'elaine-core' )    => 'huge'
								),
							...
							array(
								'type'          => 'dropdown',
								'param_name'    => 'enable_drop_border',
								'heading'       => esc_html__( 'Enable Button Drop Border', 'elaine-core' ),
								'value'      => array(
									esc_html__( 'Yes', 'elaine-core' )   => 'yes',
									esc_html__( 'No', 'elaine-core' )  => 'no'
								),
								'description'   => esc_html__( 'Enabling this option will show drop border on button', 'elaine-core' ),
						...
						elaine_edge_icon_collections()->getVCParamsArray( array(), '', true ),
						array(
							array(
								'type'       => 'colorpicker',
								'param_name' => 'color',
								'heading'    => esc_html__( 'Color', 'elaine-core' ),
								'group'      => esc_html__( 'Design Options', 'elaine-core' )
							),
							array(
								'type'       => 'colorpicker',
								'param_name' => 'hover_color',
								'heading'    => esc_html__( 'Hover Color', 'elaine-core' ),
								'group'      => esc_html__( 'Design Options', 'elaine-core' )
							),
							array(
								'type'       => 'colorpicker',
							...
							array(
								...
								'heading'    => esc_html__( 'Font Size (px)', 'elaine-core' ),
								'group'      => esc_html__( 'Design Options', 'elaine-core' )
							),
							array(
								'type'        => 'dropdown',
							...
							array(
								'type'        => 'textfield',
								'param_name'  => 'padding',
								'heading'     => esc_html__( 'Button Padding', 'elaine-core' ),
								'description' => esc_html__( 'Insert padding in format: top right bottom left (e.g. 10px 5px 10px 5px)', 'elaine-core' ),
								'dependency'  => array( 'element' => 'type', 'value' => array( 'solid', 'outline' ) ),
								'group'       => esc_html__( 'Design Options', 'elaine-core' )
							)
						)
	...
	public function render( $atts, $content = null ) {
		...
		if ( $params['html_type'] !== 'input' ) {
			...
			$params['icon_params'] = array('icon_attributes' => array('class' => 'edgtf-btn-icon'));
		}
		...
		return elaine_core_get_shortcode_module_template_part( 'templates/' . $params['html_type'], 'button', '', $params );
	}
	...
	private function getButtonStyles( $params ) {
		...
		if ( ! empty( $params['font_size'] ) ) {
			$styles[] = 'font-size: ' . elaine_edge_filter_px( $params['font_size'] ) . 'px';
		}
	...
	private function getButtonClasses( $params ) {
		...
		if ( $params['enable_drop_border']  == "yes" ){
			$buttonClasses[] = 'edgtf-btn-enabled-border';
		} else   $buttonClasses[] = 'edgtf-btn-disabled-border';
