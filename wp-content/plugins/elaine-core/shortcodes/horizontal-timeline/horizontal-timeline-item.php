...
class HorizontalTimelineItem implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'            => esc_html__( 'Elated Horizontal Timeline Item', 'elaine-core' ),
					'base'            => $this->base,
					...
					'params'          => array(
						array(
							'type'        => 'textfield',
							'param_name'  => 'date',
							'heading'     => esc_html__( 'Timeline Date', 'elaine-core' ),
							'description' => esc_html__( 'Enter date in format dd/mm/yyyy.', 'elaine-core' )
						),
						array(
							'type'       => 'attach_image',
							'param_name' => 'content_image',
							'heading'    => esc_html__( 'Content Image', 'elaine-core' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$html = elaine_core_get_shortcode_module_template_part( 'templates/horizontal-timeline-item', 'horizontal-timeline', '', $params );
