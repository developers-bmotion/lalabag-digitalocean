...
class ElaineCoreImport {
	...
	public function import_content( $file ) {
		ob_start();
		require_once( ELAINE_CORE_ABS_PATH . '/import/class.wordpress-importer.php' );
		$elaine_import = new WP_Import();
		set_time_limit( 0 );
		
		$elaine_import->fetch_attachments = $this->attachments;
		$returned_value                  = $elaine_import->import( $file );
		
		if ( is_wp_error( $returned_value ) ) {
			$this->message = esc_html__( 'An Error Occurred During Import', 'elaine-core' );
		} else {
			$this->message = esc_html__( 'Content imported successfully', 'elaine-core' );
		}
	...
	public function import_widgets( $file, $file2 ) {
		...
		foreach ( (array) $options['widgets'] as $elaine_widget_id => $elaine_widget_data ) {
			update_option( 'widget_' . $elaine_widget_id, $elaine_widget_data );
		}
		
		$this->import_sidebars_widgets( $file );
		$this->message = esc_html__( 'Widgets imported successfully', 'elaine-core' );
	}
	
	public function import_sidebars_widgets( $file ) {
		$elaine_sidebars = get_option( "sidebars_widgets" );
		unset( $elaine_sidebars['array_version'] );
		$data = $this->file_options( $file );
		
		if ( is_array( $data['sidebars'] ) ) {
			$elaine_sidebars = array_merge( (array) $elaine_sidebars, (array) $data['sidebars'] );
			unset( $elaine_sidebars['wp_inactive_widgets'] );
			$elaine_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $elaine_sidebars );
			$elaine_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $elaine_sidebars );
		}
	}
	...
	public function import_custom_sidebars( $file ) {
		...
		$this->message = esc_html__( 'Custom sidebars imported successfully', 'elaine-core' );
	}
	
	public function import_options( $file ) {
		$options       = $this->file_options( $file );
		$result        = update_option( 'edgtf_options_elaine', $options );
		$this->message = esc_html__( 'Options imported successfully', 'elaine-core' );
	}
	
	public function import_menus( $file ) {
		global $wpdb;
		$elaine_terms_table = $wpdb->prefix . "terms";
		$this->menus_data  = $this->file_options( $file );
		$menu_array        = array();
		
		foreach ( $this->menus_data as $registered_menu => $menu_slug ) {
			$term_rows = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $elaine_terms_table where slug=%s", $menu_slug ), ARRAY_A );
	...
	public function import_settings_pages( $file ) {
		...
		foreach ( $pages as $elaine_page_option => $elaine_page_id ) {
			update_option( $elaine_page_option, $elaine_page_id );
		}
	}
	...
	function edgtf_admin_import() {
		if ( elaine_core_theme_installed() ) {
			...
			$this->pagehook = add_submenu_page(
                EDGE_OPTIONS_SLUG,
				esc_html__( 'Elaine Options - Elaine Import', 'elaine-core' ), // The value used to populate the browser's title bar when the menu page is active
				esc_html__( 'Import', 'elaine-core' ),                     // The text of the menu in the administrator's sidebar
				'administrator',                                          // What roles are able to access the menu
                EDGE_OPTIONS_SLUG . $slug,                  // The ID used to bind submenu items to this menu
				array( $elaine_edge_global_Framework->getSkin(), 'renderImport' )
			);
			
			add_action( 'admin_print_scripts-' . $this->pagehook, 'elaine_edge_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $this->pagehook, 'elaine_edge_enqueue_admin_styles' );
		}
	}
	...
	function edgtf_update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( 'edgtf_options_elaine' );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( 'edgtf_options_elaine', $new_global_values );
		}
	}
