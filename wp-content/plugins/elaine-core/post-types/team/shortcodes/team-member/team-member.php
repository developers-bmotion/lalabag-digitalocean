...
class TeamMember implements Lib\ShortcodeInterface {
    ...
    public function __construct() {
        $this->base = 'edgtf_team_member';

        add_action('vc_before_init', array($this, 'vcMap'));

	    //Portfolio project id filter
	    add_filter( 'vc_autocomplete_edgtf_team_member_member_id_callback', array( &$this, 'teamMemberIdAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Portfolio project id render
	    add_filter( 'vc_autocomplete_edgtf_team_member_member_id_render', array( &$this, 'teamMemberIdAutocompleteRender', ), 10, 1 ); // Render exact portfolio. Must return an array (label,value)
    }
    ...
    public function vcMap() {
        if(function_exists('vc_map')) {
	        vc_map( array(
			        'name'                      => esc_html__( 'Team Member', 'elaine-core' ),
			        'base'                      => $this->getBase(),
			        ...
			        'params'                    => array(
                        array(
                            ...
                            'heading'     => esc_html__('Team Layout', 'elaine-core'),
                            'value'       => array(
                                esc_html__('Info Bellow', 'elaine-core')   => 'info-bellow',
                                esc_html__('Info on Hover', 'elaine-core') => 'info-hover'
                            )
                        ),
                        array(
					        'type'       => 'autocomplete',
					        'param_name' => 'member_id',
					        'heading'    => esc_html__( 'Select Team Member', 'elaine-core' ),
					        'settings'   => array(
						        'sortable'      => true,
						        'unique_values' => true
					        ),
					        'description' => esc_html__( 'If you left this field empty then project ID will be of the current page', 'elaine-core' )
				        )
			        )
		        )
	        );
    ...
    public function render($atts, $content = null) {
        ...
        $params['position'] = get_post_meta($params['member_id'], 'edgtf_team_member_position', true);
        $params['birth_date'] = get_post_meta($params['member_id'], 'edgtf_team_member_birth_date', true);
        $params['email'] = get_post_meta($params['member_id'], 'edgtf_team_member_email', true);
        $params['phone'] = get_post_meta($params['member_id'], 'edgtf_team_member_phone', true);
        $params['address'] = get_post_meta($params['member_id'], 'edgtf_team_member_address', true);
        $params['social'] = get_post_meta($params['member_id'], 'edgtf_team_member_social', true);
        $params['resume'] = get_post_meta($params['member_id'], 'edgtf_team_member_resume', true);
        $params['excerpt'] = get_the_excerpt($params['member_id']);
        $params['team_social_icons'] = $this->getTeamSocialIcons($params['member_id']);

        $html = elaine_core_get_cpt_shortcode_module_template_part('team', 'team-member', $params['team_member_layout'], '', $params);
	...
	public function teamMemberIdAutocompleteRender( $query ) {
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $team ) ) {
                ...
                $team_title_display = '';
				if ( ! empty( $team_title ) ) {
                    $team_title_display = ' - ' . esc_html__( 'Title', 'elaine-core' ) . ': ' . $team_title;
				}

                $team_id_display = esc_html__( 'Id', 'elaine-core' ) . ': ' . $team_id;
