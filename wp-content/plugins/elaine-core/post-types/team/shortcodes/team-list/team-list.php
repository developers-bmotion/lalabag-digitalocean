...
class TeamList implements Lib\ShortcodeInterface {
    ...
    public function __construct() {
	    ...
	    add_filter( 'vc_autocomplete_edgtf_team_list_selected_members_callback', array( &$this, 'teamIdAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Team selected projects render
	    add_filter( 'vc_autocomplete_edgtf_team_list_selected_members_render', array( &$this, 'teamIdAutocompleteRender', ), 10, 1 ); // Render exact team. Must return an array (label,value)
    }
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Team List', 'elaine-core' ),
				    'base'                      => $this->getBase(),
				    ...
				    'params'                    => array(
					    ...
					    array(
						    ...
						    'heading'     => esc_html__( 'Number of team members per page', 'elaine-core' ),
						    'description' => esc_html__( 'Set number of items for your team list. Enter -1 to show all.', 'elaine-core' ),
						    'value'       => '-1'
					    ),
					    array(
						    'type'        => 'autocomplete',
						    'param_name'  => 'category',
						    'heading'     => esc_html__( 'One-Category Team List', 'elaine-core' ),
					    ...
					    array(
						    ...
						    'heading'     => esc_html__( 'Show Only Members with Listed IDs', 'elaine-core' ),
						    'settings'    => array(
							    'multiple'      => true,
							    'sortable'      => true,
							    'unique_values' => true
					    ...
					    array(
						    ...
						    'heading'     => esc_html__( 'Team Member Layout', 'elaine-core' ),
						    'value'       => array(
	...
	public function render( $atts, $content = null ) {
		...
		$html = elaine_core_get_cpt_shortcode_module_template_part( 'team', 'team-list', 'team-holder', '', $params, $additional_params );
	...
	public function teamCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['team_category_title'] ) > 0 ) ? esc_html__( 'Category', 'elaine-core' ) . ': ' . $value['team_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $team_category ) ) {
				...
				if ( ! empty( $team_category_title ) ) {
					$team_category_title_display = esc_html__( 'Category', 'elaine-core' ) . ': ' . $team_category_title;
				}
