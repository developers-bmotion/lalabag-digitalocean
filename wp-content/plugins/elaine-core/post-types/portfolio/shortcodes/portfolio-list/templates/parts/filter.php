<?php if($filter == 'yes') {
	...
	?>
	<div class="edgtf-pl-filter-holder" <?php elaine_edge_inline_style($filter_holder_styles); ?>>
		<div class="edgtf-plf-inner">
			<?php
			if(is_array($filter_categories) && count($filter_categories)){ ?>
				<ul <?php elaine_edge_inline_style($filter_styles); ?>>
					<li class="edgtf-pl-filter" data-filter="">
						<span><?php esc_html_e('Show all', 'elaine-core')?></span>
					</li>
