...
if(is_array($media) && count($media)) { ?>
   ...
   <?php foreach ($media as $key => $single_media) {
		...
		<a itemprop="url" <?php elaine_edge_class_attribute($link_class);?> data-rel="prettyPhoto[rel-gallery-item-<?php echo get_the_ID() . '-' . esc_attr($unique_id); ?>]" href="<?php echo esc_url($image_src); ?>" title="<?php echo get_the_title(get_the_ID()); ?>"></a>
		<?php }
}