...
class PortfolioList implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map( array(
					'name'     => esc_html__( 'Portfolio List', 'elaine-core' ),
					'base'     => $this->getBase(),
					...
					'params'   => array(
						array(
							'type'        => 'dropdown',
							'param_name'  => 'type',
							'heading'     => esc_html__( 'Portfolio List Template', 'elaine-core' ),
							'value'       => array(
								esc_html__( 'Gallery', 'elaine-core' ) => 'gallery',
								esc_html__( 'Masonry', 'elaine-core' ) => 'masonry'
							),
							'save_always' => true,
							'admin_label' => true
						),
						...
						array(
							...
							'description' => esc_html__( 'Default value is Three', 'elaine-core' ),
							'save_always' => true
						),
						array(
							'type'        => 'dropdown',
						...
						array(
							...
							'heading'     => esc_html__( 'Number of Portfolios Per Page', 'elaine-core' ),
							'description' => esc_html__( 'Set number of items for your portfolio list. Enter -1 to show all.', 'elaine-core' ),
							'value'       => '-1'
						),
						array(
							'type'        => 'dropdown',
							...
							'description' => esc_html__( 'Set image proportions for your portfolio list.', 'elaine-core' ),
							'dependency'  => array( 'element' => 'type', 'value' => array( 'gallery' ) )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'enable_fixed_proportions',
							'heading'     => esc_html__( 'Enable Fixed Image Proportions', 'elaine-core' ),
							'value'       => array_flip( elaine_edge_get_yes_no_select_array( false ) ),
							'description' => esc_html__( 'Set predefined image proportions for your masonry portfolio list. This option will apply image proportions you set in Portfolio Single page - dimensions for masonry option.', 'elaine-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Pagination Type', 'elaine-core' ),
							'value'      => array(
								esc_html__( 'None', 'elaine-core' )            => 'no-pagination',
								esc_html__( 'Standard', 'elaine-core' )        => 'standard',
								esc_html__( 'Load More', 'elaine-core' )       => 'load-more',
								esc_html__( 'Infinite Scroll', 'elaine-core' ) => 'infinite-scroll'
							),
							'group'      => esc_html__( 'Additional Features', 'elaine-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'load_more_top_margin',
							'heading'    => esc_html__( 'Load More Top Margin (px or %)', 'elaine-core' ),
						...
						array(
							'type'       => 'dropdown',
							'param_name' => 'filter',
							'heading'    => esc_html__( 'Enable Category Filter', 'elaine-core' ),
							'value'      => array_flip( elaine_edge_get_yes_no_select_array( false ) ),
							'group'      => esc_html__( 'Additional Features', 'elaine-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Filter Order By', 'elaine-core' ),
							'value'      => array(
								esc_html__( 'Name', 'elaine-core' )  => 'name',
								esc_html__( 'Count', 'elaine-core' ) => 'count',
								esc_html__( 'Id', 'elaine-core' )    => 'id',
								esc_html__( 'Slug', 'elaine-core' )  => 'slug'
							),
						...
						array(
							...
							'heading'    => esc_html__( 'Filter Text Transform', 'elaine-core' ),
							'value'      => array_flip( elaine_edge_get_text_transform_array( true ) ),
							'dependency' => array( 'element' => 'filter', 'value' => array( 'yes' ) ),
							'group'      => esc_html__( 'Additional Features', 'elaine-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Filter Bottom Margin (px or %)', 'elaine-core' ),
							'dependency' => array( 'element' => 'filter', 'value' => array( 'yes' ) ),
							'group'      => esc_html__( 'Additional Features', 'elaine-core' )
						),
						array(
							...
							'heading'     => esc_html__( 'Enable Article Animation', 'elaine-core' ),
							'value'       => array_flip( elaine_edge_get_yes_no_select_array( false ) ),
							'description' => esc_html__( 'Enabling this option you will enable appears animation for your portfolio list items', 'elaine-core' ),
							'group'       => esc_html__( 'Additional Features', 'elaine-core' )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$additional_params['holder_data']          = elaine_edge_get_holder_data_for_cpt( $params, $additional_params );
		...
		$html = elaine_core_get_cpt_shortcode_module_template_part( 'portfolio', 'portfolio-list', 'portfolio-holder', $params['type'], $params, $additional_params );
	...
	public function getHolderClasses( $params, $args ) {
		...
		$classes[] = $params['enable_fixed_proportions'] === 'yes' ? 'edgtf-fixed-masonry-items' : '';
	...
	public function getHolderInnerClasses( $params ) {
		...
		$classes[] = $params['portfolio_slider_on'] === 'yes' ? 'edgtf-owl-slider edgtf-list-is-slider' : '';
	...
	public function getArticleClasses( $params ) {
		...
		$classes[] = ! empty( $masonry_size ) && $type === 'masonry' ? 'edgtf-masonry-size-' . esc_attr( $masonry_size ) : '';
	...
	public function getStandardContentStyles( $params ) {
		...
		if ( ! empty( $margin_top ) ) {
			if ( elaine_edge_string_ends_with( $margin_top, '%' ) || elaine_edge_string_ends_with( $margin_top, 'px' ) ) {
		...
		if ( ! empty( $margin_bottom ) ) {
			if ( elaine_edge_string_ends_with( $margin_bottom, '%' ) || elaine_edge_string_ends_with( $margin_bottom, 'px' ) ) {
	...
	public function getLoadMoreStyles( $params ) {
		...
		if ( ! empty( $params['load_more_top_margin'] ) ) {
			...
			if ( elaine_edge_string_ends_with( $margin, '%' ) || elaine_edge_string_ends_with( $margin, 'px' ) ) {
				$styles[] = 'margin-top: ' . $margin;
			} else {
				$styles[] = 'margin-top: ' . elaine_edge_filter_px( $margin ) . 'px';
			}
		}
	...
	public function getFilterHolderStyles( $params ) {
		...
		if ( ! empty( $params['filter_bottom_margin'] ) ) {
			...
			if ( elaine_edge_string_ends_with( $margin, '%' ) || elaine_edge_string_ends_with( $margin, 'px' ) ) {
				...
				$styles[] = 'margin-bottom: ' . elaine_edge_filter_px( $margin ) . 'px';
			}
		}
	...
	public function getItemLink() {
		...
		return apply_filters( 'elaine_edge_filter_portfolio_external_link', $portfolio_link );
	}
	...
	public function getItemLinkTarget() {
		...
		return apply_filters( 'elaine_edge_filter_portfolio_external_link_target', $portfolio_link_target );
	}
