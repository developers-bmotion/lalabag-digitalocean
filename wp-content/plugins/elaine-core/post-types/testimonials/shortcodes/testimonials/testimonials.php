...
namespace ElaineCore\CPT\Shortcodes\Testimonials;
...
class Testimonials implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Testimonials', 'elaine-core' )
						),
						array(
							'type'       => 'dropdown',
							...
							'save_always' => true,
							'description' => esc_html__('Number of Testimonials visible at the same time', 'elaine-core')
						),
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
							'heading'     => esc_html__( 'Category', 'elaine-core' ),
	...
	public function render( $atts, $content = null ) {
        ...
        return elaine_core_get_cpt_shortcode_module_template_part( 'testimonials', 'testimonials', 'testimonials-' . $params['type'], '', $params );
	...
	private function getHolderClasses( $params ) {
		...
		//$holderClasses[] = ! empty( $params['number_of_columns'] ) ? 'edgtf-' . $params['number_of_columns'] . '-columns' : 'edgtf-' . $params['number_of_columns'] . '-columns';
	...
	private function getSliderData( $params ) {
		...
		$slider_data['data-number-visible']      = ! empty( $params['number_of_visible_items'] ) ? $params['number_of_visible_items'] : '';
	...
	public function testimonialsCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['testimonials_category_title'] ) > 0 ) ? esc_html__( 'Category', 'elaine-core' ) . ': ' . $value['testimonials_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $testimonials_category ) ) {
				...
				if ( ! empty( $testimonials_category_title ) ) {
					$testimonials_category_title_display = esc_html__( 'Category', 'elaine-core' ) . ': ' . $testimonials_category_title;
				}
