...
class VI_WORDPRESS_LUCKY_WHEEL_Frontend_Frontend {
	...
	display: block;">
			...
			wp_mail( $admin_email_address, $subject1, $content1, $header );
		}
	...
	public function wp_print_styles() {
		if ( $this->is_mobile ) {
			?>
            <style id="wordpress-lucky-wheel-frontend-wplwl_user_lucky"></style>
			<?php
		}
	}
	...
	public function frontend_enqueue() {
		...
		if ( $this->is_mobile ) {
			wp_enqueue_script( 'wordpress-lucky-wheel-frontend-javascript', VI_WORDPRESS_LUCKY_WHEEL_JS . 'wordpress-lucky-wheel-mobile.js', array( 'jquery' ), VI_WORDPRESS_LUCKY_WHEEL_VERSION );
		} else {
			wp_enqueue_script( 'wordpress-lucky-wheel-frontend-javascript', VI_WORDPRESS_LUCKY_WHEEL_JS . 'wordpress-lucky-wheel.js', array( 'jquery' ), VI_WORDPRESS_LUCKY_WHEEL_VERSION );
		}
		$font = '';
		if ( $this->settings->get_params( 'wheel_wrap', 'font' ) ) {
			$font = $this->settings->get_params( 'wheel_wrap', 'font' );
			wp_enqueue_style( 'wordpress-lucky-wheel-google-font-' . strtolower( str_replace( '+', '-', $font ) ), '//fonts.googleapis.com/css?family=' . $font . ':300,400,700' );
		...
		switch ( $this->background_effect ) {
			case 'snowflakes':
				wp_enqueue_style( 'wordpress-lucky-wheel-frontend-style-snowflakes', VI_WORDPRESS_LUCKY_WHEEL_CSS . 'snowflakes.css', array(), VI_WORDPRESS_LUCKY_WHEEL_VERSION );
				break;
			case 'snowflakes-1':
				wp_enqueue_style( 'wordpress-lucky-wheel-frontend-style-snowflakes-1', VI_WORDPRESS_LUCKY_WHEEL_CSS . 'snowflakes-1.css', array(), VI_WORDPRESS_LUCKY_WHEEL_VERSION );
				break;
			case 'snowflakes-2-1':
			case 'snowflakes-2-2':
			case 'snowflakes-2-3':
				wp_enqueue_style( 'wordpress-lucky-wheel-frontend-style-snowflakes-2', VI_WORDPRESS_LUCKY_WHEEL_CSS . 'snowflakes-2.css', array(), VI_WORDPRESS_LUCKY_WHEEL_VERSION );
				break;
			default:
		}
		wp_enqueue_style( 'wordpress-lucky-wheel-frontend-style', VI_WORDPRESS_LUCKY_WHEEL_CSS . 'wordpress-lucky-wheel.css', array(), VI_WORDPRESS_LUCKY_WHEEL_VERSION );
		...
		if ( 'on' == $this->settings->get_params( 'wheel', 'show_full_wheel' ) ) {
			$css .= '.wplwl_lucky_wheel_content .wheel_content_left{margin-left:0 !important;}';
			$css .= '.wplwl_lucky_wheel_content .wheel_content_right{width:48% !important;}';
//			$css .= '.wplwl_lucky_wheel_content .wheel_content_right .wplwl_user_lucky{max-width:300px !important;}';
		}
		...
		if ( $this->settings->get_params( 'wheel_wrap', 'pointer_color' ) ) {
			$css .= '.wplwl_pointer:before{color:' . $this->settings->get_params( 'wheel_wrap', 'pointer_color' ) . ';}';
		}
		//wheel wrap design
		$css .= '.wheel_content_right>.wplwl_user_lucky>.wplwl_spin_button{';
		...
		switch ( $this->settings->get_params( 'notify', 'show_again_unit' ) ) {
			case 's':
				$limit_time_warning = sprintf( esc_html__( 'You can only spin 1 time every %s seconds', 'wordpress-lucky-wheel' ), $this->settings->get_params( 'notify', 'show_again' ) );
				break;
			case 'm':
				$limit_time_warning = sprintf( esc_html__( 'You can only spin 1 time every %s minutes', 'wordpress-lucky-wheel' ), $this->settings->get_params( 'notify', 'show_again' ) );
				break;
			case 'h':
				$limit_time_warning = sprintf( esc_html__( 'You can only spin 1 time every %s hours', 'wordpress-lucky-wheel' ), $this->settings->get_params( 'notify', 'show_again' ) );
				break;
			case 'd':
				$limit_time_warning = sprintf( esc_html__( 'You can only spin 1 time every %s days', 'wordpress-lucky-wheel' ), $this->settings->get_params( 'notify', 'show_again' ) );
				break;
		...
		wp_localize_script( 'wordpress-lucky-wheel-frontend-javascript', '_wplwl_get_email_params', array(
	...
	public function draw_wheel() {
		...
		if ( in_array( $this->background_effect, array(
			...
			?>
            <div class="wplwl-overlay wplwl-background-effect-snowflakes-2 <?php echo 'wplwl-background-effect-' . $this->background_effect ?>">
                <i></i>
            </div>
			<?php
		} else {
			?>
            <div class="wplwl-overlay">
            </div>
			<?php
		}
		?>
        <input id="wplwl_center_image" type="hidden" value="<?php echo $center_image ?>">
		<?php
		if ( $this->is_mobile ) {
			?>
			...
			if ( in_array( $this->background_effect, array(
				...
				echo ' wplwl-background-effect-snowflakes-2 wplwl-background-effect-' . $this->background_effect;
			}
			?>">
			<?php
			switch ( $this->background_effect ) {
		...
		?>
        <canvas id="wplwl_popup_canvas" class="<?php esc_attr_e( $wheel_icon_class ) ?>" width="64"
                height="64"></canvas>
		<?php
	}
	...
	public function get_email() {
		...
		if ( $the_query->have_posts() ) {
			while ( $the_query->have_posts() ) {
				$the_query->the_post();
				$email_id   = get_the_ID();
				$old_mobile = get_post_meta( $email_id, 'wplwl_email_mobile', true );
				if ( ! $old_mobile && $mobile ) {
					update_post_meta( $email_id, 'wplwl_email_mobile', $mobile );
				}
				...
				if ( $spin_meta['spin_num'] >= $this->settings->get_params( 'general', 'spin_num' ) ) {
					...
					if ( $wait_day ) {
						$wait_return = sprintf( esc_html__( '%s days %s hours %s minutes %s seconds', 'wordpress-lucky-wheel' ), $wait_day, $wait_hour, $wait_min, $wait_sec );
					} elseif ( $wait_hour ) {
						$wait_return = sprintf( esc_html__( '%s hours %s minutes %s seconds', 'wordpress-lucky-wheel' ), $wait_hour, $wait_min, $wait_sec );
					} elseif ( $wait_min ) {
						$wait_return = sprintf( esc_html__( '%s minutes %s seconds', 'wordpress-lucky-wheel' ), $wait_min, $wait_sec );
					}
					$allow = esc_html__( 'You have to wait ', 'wordpress-lucky-wheel' ) . ( $wait_return ) . esc_html__( ' to be able to spin again.', 'wordpress-lucky-wheel' );
				} else {
					$allow = 'yes';
					$spin_meta['spin_num'] ++;
					update_post_meta(
					...
					if ( $wheel['prize_type'][ $stop ] != 'non' ) {
						...
						$this->send_email( $email, $name, $mobile, $code, $wheel_label, $language );
