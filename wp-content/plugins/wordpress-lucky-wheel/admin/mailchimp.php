...
class VI_WORDPRESS_LUCKY_WHEEL_Admin_Mailchimp {
	protected $settings;
	protected $api_key;
	...
	function get_lists() {
		if ( $this->api_key ) {
			...
			if ( $dash_position !== false ) {
				$api_url = 'https://' . substr( $this->api_key, $dash_position + 1 ) . '.api.mailchimp.com/3.0/lists?fields=lists.name,lists.id';
	...
	function add_email( $email,$list_id, $fname='', $lname='',$phone='' ,$birthday='') {
		if ( $this->api_key && $list_id ) {
			$data = array(
				'email_address' => $email,
				'status'        => 'subscribed' ,
