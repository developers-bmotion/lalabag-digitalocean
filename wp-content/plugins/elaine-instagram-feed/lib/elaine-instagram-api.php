...
include 'elaine-instagram-helper.php';

/**
 * Class ElaineInstagramApi
 */
class ElaineInstagramApi {
	/**
	...
	private $apiURL;
	/**
	 * @var ElaineInstagramApi instance of current class
	 */
	private static $instance;
	
	/**
	 * Helper object
	 * @var ElaineInstagramHelper
	 */
	private $helper;
	
	const CODE_OPTION_NAME = 'elaine_instagram_code';
	const USER_ID_OPTION_NAME = 'elaine_instagram_user_id';
	const ACCESS_TOKEN_OPTION_NAME = 'elaine_instagram_access_token';
	const CODE_REDIRECT_URI_OPTION_NAME = 'elaine_instagram_code_redirect_uri';
	...
	public function __construct() {
		...
		$this->redirectUri  = defined( 'EDGE_PROFILE_SLUG' ) ? 'http://demo.' . EDGE_PROFILE_SLUG . '-themes.com/instagram-app/instagram-redirect.php' : '';
		...
		$this->helper       = new ElaineInstagramHelper();
	}
	 ...
	 * @return string
	 *
	 * @see ElaineInstagramApi::buildCurrentPageURI()
	 */
	 ...
	 *
	 * @see ElaineInstagramApi::fetchData()
	 */
	 ...
	 * @return stdClass
	 *
	 * @see ElaineInstagramApi::getAccessToken()
	 */
	private function fetchData( $limit = '', $transient = array( 'use_transient' => false ) ) {
		...
		if ( $this->useTransients( $transient ) && get_transient( $transient['transient_name'] ) ) {
			...
			if ( is_wp_error( $httpResponse ) || ( isset( $httpBody['meta']['code'] ) && $httpBody['meta']['code'] !== 200 ) ) {
				...
				$returnObject->message = esc_html__('Can\'t connect with Instagram', 'elaine-instagram-feed');
		...
		$returnObject->message = esc_html__('Success', 'elaine-instagram-feed');
	...
	public function getAccessToken() {
		...
		if ( empty( $this->code ) ) {
			$returnObject->status  = 'error';
			$returnObject->message = esc_html__('User hasn\'t connected with Instagram', 'elaine-instagram-feed');
			...
			if ( is_wp_error( $httpResponse ) || ( isset( $httpResponse['response']['code'] ) && $httpResponse['response']['code'] !== 200 ) ) {
				$returnObject->status  = 'error';
				$returnObject->message = esc_html__('Can\'t connect with Instagram API', 'elaine-instagram-feed');
			...
			if ( isset( $responseBody['access_token'] ) && isset( $responseBody['user']['id'] ) ) {
				...
				$returnObject->status  = 'error';
				$returnObject->message = esc_html__('Can\'t get Instagram access token', 'elaine-instagram-feed');
		...
		//prepare return object and return it
		$returnObject->status  = 'ok';
		$returnObject->message = esc_html__('Stored access token', 'elaine-instagram-feed');
