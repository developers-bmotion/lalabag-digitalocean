...
if ( ! function_exists( 'elaine_edge_add_custom_nav_fields' ) ) {
	function elaine_edge_add_custom_nav_fields( $menu_item ) {
	...
	add_filter( 'wp_setup_nav_menu_item', 'elaine_edge_add_custom_nav_fields' );
}
 ...
 * @return      void
 */
if ( ! function_exists( 'elaine_edge_update_custom_nav_fields' ) ) {
	function elaine_edge_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
		$check = array(
			'anchor',
			'nolink',
			'hide',
	...
	// save menu custom fields
	add_action( 'wp_update_nav_menu_item', 'elaine_edge_update_custom_nav_fields', 10, 3 );
}
 ...
 * @return      void
 */
if ( ! function_exists( 'elaine_edge_edit_walker' ) ) {
	function elaine_edge_edit_walker( $walker, $menu_id ) {
		return 'ElaineEdgeClass_Walker_Nav_Menu_Edit_Custom';
	}
	
	// edit menu walker
	add_filter( 'wp_edit_nav_menu_walker', 'elaine_edge_edit_walker', 10, 2 );
}

if ( ! function_exists( 'elaine_edge_include_custom_walkers' ) ) {
	function elaine_edge_include_custom_walkers() {
		...
		do_action( 'elaine_edge_action_include_custom_walkers_nav' );
	}
	
	add_action( 'after_setup_theme', 'elaine_edge_include_custom_walkers' );
}