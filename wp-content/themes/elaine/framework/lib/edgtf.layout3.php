...
/*
   Class: ElaineEdgeClassMultipleImages
   A class that initializes Edge Multiple Images
*/

class ElaineEdgeClassMultipleImages implements iElaineEdgeInterfaceRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$elaine_edge_global_Framework->edgtMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="edgtf-gallery-uploader">
								<a class="edgtf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'elaine' ); ?></a>
								<a class="edgtf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'elaine' ); ?></a>
...
class ElaineEdgeClassTwitterFramework implements iElaineEdgeInterfaceRender {
	public function render( $factory ) {
		$twitterApi = ElaineTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'elaine' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'elaine' ) : esc_html__( 'Connect with Twitter', 'elaine' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="edgtf-page-form-section" id="edgtf_enable_social_share">
			<div class="edgtf-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'elaine' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'elaine' ); ?></p>
			</div>
			<div class="edgtf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class ElaineEdgeClassInstagramFramework implements iElaineEdgeInterfaceRender {
	public function render( $factory ) {
		$instagram_api = ElaineInstagramApi::getInstance();
		$message       = '';
		
		//if code wasn't saved to database
		if ( ! get_option( 'elaine_instagram_code' ) ) {
			...
			if ( ! empty( $_GET['code'] ) ) {
				...
				$message = esc_html__( 'You have successfully connected with your Instagram account. If you have any issues fetching data from Instagram try reconnecting.', 'elaine' );
		...
		$buttonText = $instagram_api->hasUserConnected() ? esc_html__( 'Re-connect with Instagram', 'elaine' ) : esc_html__( 'Connect with Instagram', 'elaine' );
		?>
		<?php if ( $message !== '' ) { ?>
			<div class="alert alert-success" style="margin-top: 20px;">
				<span><?php echo esc_html( $message ); ?></span>
		...
		<div class="edgtf-page-form-section" id="edgtf_enable_social_share">
			<div class="edgtf-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'elaine' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'elaine' ); ?></p>
			</div>
			<div class="edgtf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class ElaineEdgeClassRepeater implements iElaineEdgeInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="edgtf-repeater-table-heading">
					<div class="edgtf-repeater-fields-holder">
						<div class="edgtf-repeater-table-cell edgtf-repeater-sort"><?php esc_html_e( 'Order', 'elaine' ) ?></div>
						...
						<div class="edgtf-repeater-table-cell edgtf-repeater-remove"><?php esc_html_e( 'Remove', 'elaine' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="edgtf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="edgtf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = elaine_edge_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class ElaineEdgeClassFieldAddress extends ElaineEdgeClassFieldType {
					...
					<div class="row">
						<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
							...
							if ( empty( $google_maps_api_key ) ) { ?>
								<p class="description"><?php esc_html_e( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Elaine Options', 'elaine' ); ?></p>
							<?php } ?>
...
class ElaineEdgeClassFieldIcon extends ElaineEdgeClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		$class = '';
		...
		$icon_packs        = elaine_edge_icon_collections()->getIconCollectionsEmpty();
