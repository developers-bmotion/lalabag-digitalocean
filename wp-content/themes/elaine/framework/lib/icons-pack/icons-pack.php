...
foreach ( glob( EDGE_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( elaine_edge_is_customizer_item_enabled( $module_load, 'elaine_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'elaine_edge_get_icon_pack_collections' ) ) {
	function elaine_edge_get_icon_pack_collections() {
		$options = apply_filters( 'elaine_edge_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'elaine_edge_get_icon_pack_collections' );
}

if ( ! function_exists( 'elaine_edge_get_icon_pack_options' ) ) {
	function elaine_edge_get_icon_pack_options() {
		$options = apply_filters( 'elaine_edge_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'elaine_edge_get_icon_pack_options' );
}

if ( ! function_exists( 'elaine_edge_get_icon_pack_social_options' ) ) {
	function elaine_edge_get_icon_pack_social_options() {
		$options = apply_filters( 'elaine_edge_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'elaine_edge_get_icon_pack_social_options' );
}

/*
 * Class: ElaineEdgeClassIconCollections
 */
class ElaineEdgeClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = elaine_edge_get_icon_pack_collections();
		$this->iconPackOptions       = elaine_edge_get_icon_pack_options();
		$this->iconPackSocialOptions = elaine_edge_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = elaine_edge_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'elaine' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'elaine' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'elaine' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'elaine' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'elaine' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'elaine' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'elaine' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'elaine' )       => 'fa-lg',
			esc_html__( 'Small', 'elaine' )      => 'fa-2x',
			esc_html__( 'Medium', 'elaine' )     => 'fa-3x',
			esc_html__( 'Large', 'elaine' )      => 'fa-4x',
			esc_html__( 'Very Large', 'elaine' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iElaineEdgeInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'elaine' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'elaine' ) ] = '';
