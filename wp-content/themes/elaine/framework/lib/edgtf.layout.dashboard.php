...
/*
   Class: ElaineEdgeClassDashboardForm
   A class that initializes Edge Dashboard Container
*/
class ElaineEdgeClassDashboardForm implements iElaineEdgeInterfaceLayoutNode, iElaineEdgeInterfaceRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
...
/*
   Class: ElaineEdgeClassDashboardGroup
   A class that initializes ElaineEdgeClass Group Field
*/
class ElaineEdgeClassDashboardGroup implements iElaineEdgeInterfaceLayoutNode, iElaineEdgeInterfaceRender{
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: ElaineEdgeClassDashboardTitle
   A class that initializes Dashboard Title
*/
class ElaineEdgeClassDashboardTitle implements iElaineEdgeInterfaceRender {
	private $name;
	private $title;
...
/*
   Class: ElaineEdgeClassDashboardField
   A class that initializes ElaineEdgeClass Front Field
*/

class ElaineEdgeClassDashboardField implements iElaineEdgeInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class ElaineEdgeClassDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class ElaineEdgeClassDashboardFieldText extends ElaineEdgeClassDashboardFieldType {
...
class ElaineEdgeClassDashboardFieldTextArea extends ElaineEdgeClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class ElaineEdgeClassDashboardFieldImage extends ElaineEdgeClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
		...
		<div class="edgtf-dashboard-field-holder <?php echo esc_attr($class); ?>">
			...
			<div class="edgtf-dashboard-gallery-holder">
				...
				<div class="edgtf-dashboard-gallery-uploader">
					<?php 
					if ( elaine_membership_theme_installed() ) {
						echo elaine_edge_get_button_html( array(
							'text'      => esc_html__( 'Upload', 'elaine' ),
							'custom_class' => 'edgtf-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="edgtf-btn edgtf-btn-medium edgtf-btn-solid edgtf-dashboard-gallery-upload"><span class="edgtf-btn-text">' . esc_html__( 'Upload', 'elaine' ) . '</span></a>';
					} ?>
					<input class="edgtf-dashboard-gallery-upload-hidden" type="file" name="<?php echo esc_attr( $name ); ?>" id="<?php echo esc_attr( $name ); ?>"
					       value="">
					<input type="hidden" class="edgtf-dashboard-media-hidden" name="<?php echo esc_attr( $hidden_name ); ?>" value="<?php echo esc_attr($value);?>"/>
					<?php if ($value !== '' && $value !== false) { ?>
						<button class="edgtf-btn edgtf-btn-solid edgtf-dashboard-remove-image"><?php esc_html_e('Remove Media','elaine'); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class ElaineEdgeClassDashboardFieldGallery extends ElaineEdgeClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class ElaineEdgeClassDashboardFieldSelect extends ElaineEdgeClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class ElaineEdgeClassDashboardFieldDate extends ElaineEdgeClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class ElaineEdgeClassDashboardFieldIcon extends ElaineEdgeClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = elaine_edge_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = elaine_edge_icon_collections()->getIconCollectionsKeys();
				...
				<div class="edgtf-dashboard-item">
					...
					<div class="edgtf-dashboard-icon-holder">
						...
						<?php foreach ( $icons_collections as $icons_collection ) {
							$icons_param = elaine_edge_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
	                        $style       = 'display: none';
							...
							<div class="edgtf-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                                <select name="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>" id="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>">
									<?php
									$icons = elaine_edge_icon_collections()->getIconCollection( $icons_collection );
...
class ElaineEdgeClassDashboardFieldColor extends ElaineEdgeClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class ElaineEdgeClassDashboardFieldCheckBoxGroup extends ElaineEdgeClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class ElaineEdgeClassDashboardFieldAddress extends ElaineEdgeClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
				...
				<div class="edgtf-dashboard-item col-lg-<?php echo esc_attr($col_width); ?>">
					<div class="edgtf-dashboard-input-holder">
						...
						<?php if ($description !== '') { ?>
							<p class="description"><?php echo esc_html( $description ); ?></p>
						<?php } ?>
						<input class="edgtf-dashboard-input" type="text" name="<?php echo esc_attr($name);?>" id="<?php echo esc_attr($id);?>" value="<?php echo esc_attr( htmlspecialchars( $value ) ); ?>">
						<div class="map_canvas"></div>
						<a id="reset" href="#" style="display:none;"><?php esc_html_e( 'Reset Marker', 'elaine' ); ?></a>
					</div>
				</div>
			</div>
		</div>
...
class ElaineEdgeClassDashboardRepeater implements iElaineEdgeInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name        = $name;
		$this->num_of_rows = 1;
		$this->button_text = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'elaine' );
			...
			<?php if ($this->table_layout) { ?>
				<div class="edgtf-dashboard-repeater-table-heading">
					<div class="edgtf-dashboard-repeater-fields-holder">
						<div class="edgtf-dashboard-repeater-table-cell edgtf-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'elaine' ) ?></div>
						...
						<div class="edgtf-dashboard-repeater-table-cell edgtf-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'elaine' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class ElaineEdgeClassDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array()) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new ElaineEdgeClassDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'textarea':
				$field = new ElaineEdgeClassDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'date':
				$field = new ElaineEdgeClassDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new ElaineEdgeClassDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'gallery':
				$field = new ElaineEdgeClassDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new ElaineEdgeClassDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new ElaineEdgeClassDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new ElaineEdgeClassDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'checkboxgroup':
				$field = new ElaineEdgeClassDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'address':
				$field = new ElaineEdgeClassDashboardFieldAddress();
