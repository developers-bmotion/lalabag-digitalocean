...
/*
   Class: ElaineEdgeClassTaxonomyField
   A class that initializes ElaineEdgeClass Taxonomy Field
*/

class ElaineEdgeClassTaxonomyField implements iElaineEdgeInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
	...
	function __construct( $type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden_property = "", $hidden_values = array() ) {
		...
		add_filter( 'elaine_edge_filter_taxonomy_fields', array( $this, 'addFieldForEditSave' ) );
	}
...
abstract class ElaineEdgeClassTaxonomyFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array() );
}

class ElaineEdgeClassTaxonomyFieldText extends ElaineEdgeClassTaxonomyFieldType {
...
class ElaineEdgeClassTaxonomyFieldImage extends ElaineEdgeClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false) {
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			<div class="form-field">
				<label for="<?php echo esc_attr( $name ); ?>"><?php echo esc_html( $label ); ?></label>
				...
				<p>
					<input type="button" class="button button-secondary edgtf-tax-media-add" name="edgtf-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'elaine' ); ?>"/>
					<input type="button" class="button button-secondary edgtf-tax-media-remove" name="edgtf-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'elaine' ); ?>"/>
				</p>
			...
			<tr class="form-field" <?php if ($hidden) { ?> style="display: none"<?php } ?>>
				...
				<td>
					...
					<p>
						<input type="button" class="button button-secondary edgtf-tax-media-add" name="edgtf-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'elaine' ); ?>"/>
						<input data-termid="<?php echo esc_attr( $_GET['tag_ID'] ); ?>" data-taxonomy="<?php echo esc_attr( $taxonomy ); ?>" type="button" class="button button-secondary edgtf-tax-media-remove" name="edgtf-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'elaine' ); ?>"/>
					</p>
...
class ElaineEdgeClassTaxonomyFieldSelect extends ElaineEdgeClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class ElaineEdgeClassTaxonomyFieldIcon extends ElaineEdgeClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		...
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				...
				<div class="form-field edgt-icon-collection-holder" style="display: none" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
					<label for="<?php echo esc_attr( $name ) . '_icon'; ?>"><?php esc_html_e( 'Icon', 'elaine' ); ?></label>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				...
				<tr class="form-field edgt-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
					<th scope="row"><?php esc_html_e( 'Icon', 'elaine' ); ?></th>
					<td>
						<select name="<?php echo esc_attr( $name . '_' . $icons_param ) ?>" id="<?php echo esc_attr( $name . '_' . $icons_param ) ?>">
							<?php
							$icons      = elaine_edge_icon_collections()->getIconCollection( $icons_collection );
...
class ElaineEdgeClassTaxonomyFieldColor extends ElaineEdgeClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class ElaineEdgeClassTaxonomyFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new ElaineEdgeClassTaxonomyFieldText();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'image':
				$field = new ElaineEdgeClassTaxonomyFieldImage();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'selectblank':
				$field = new ElaineEdgeClassTaxonomyFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'icon':
				$field = new ElaineEdgeClassTaxonomyFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'color':
				$field = new ElaineEdgeClassTaxonomyFieldColor();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
