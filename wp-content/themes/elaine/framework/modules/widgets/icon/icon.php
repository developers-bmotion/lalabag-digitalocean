...
class ElaineEdgeClassIconWidget extends ElaineEdgeClassWidget {
	public function __construct() {
		parent::__construct(
			'edgtf_icon_widget',
			esc_html__( 'Elaine Icon Widget', 'elaine' ),
			array( 'description' => esc_html__( 'Add icons to widget areas', 'elaine' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			elaine_edge_icon_collections()->getIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'icon_text',
					'title' => esc_html__( 'Icon Text', 'elaine' )
				),
				array(
					'type'  => 'textfield',
					'name'  => 'link',
				...
				array(
					'type'    => 'dropdown',
					'name'    => 'target',
					'title'   => esc_html__( 'Target', 'elaine' ),
					'options' => elaine_edge_get_link_target_array()
				),
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icon Size (px)', 'elaine' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'icon_color',
					'title' => esc_html__( 'Icon Color', 'elaine' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'icon_hover_color',
					'title' => esc_html__( 'Icon Hover Color', 'elaine' )
				),
				array(
					'type'  => 'textfield',
					'name'  => 'text_size',
					'title' => esc_html__( 'Text Size (px)', 'elaine' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'text_color',
					'title' => esc_html__( 'Text Color', 'elaine' )
				),
				array(
					'type'        => 'textfield',
					'name'        => 'margin',
	...
	public function widget( $args, $instance ) {
		...
		$data   = array();
		$data[] = ! empty( $instance['icon_hover_color'] ) ? elaine_edge_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
		$data   = implode( ' ', $data );
		...
		if ( ! empty( $instance['text_size'] ) ) {
			$text_styles[] = 'font-size: ' . elaine_edge_filter_px( $instance['text_size'] ) . 'px';
		}
		...
		if ( ! empty( $instance['icon_pack'] ) ) {
			...
			if ( ! empty( $icon_class ) ) {
				...
				$icon_holder_html = '<span class="edgtf-icon-element ' . esc_attr( $icon_class ) . '" ' . elaine_edge_get_inline_style( $icon_styles ) . '></span>';
			}
		}
		...
		if ( ! empty( $instance['icon_text'] ) ) {
			$icon_text_html = '<span class="edgtf-icon-text ' . esc_attr( $icon_text_class ) . '" ' . elaine_edge_get_inline_style( $text_styles ) . '>' . esc_html( $instance['icon_text'] ) . '</span>';
		}
		?>
		
		<a <?php elaine_edge_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo elaine_edge_get_inline_style( $holder_styles ); ?>>
