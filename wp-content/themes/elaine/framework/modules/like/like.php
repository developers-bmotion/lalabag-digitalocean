...
class ElaineEdgeClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_elaine_edge_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_elaine_edge_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'elaine' );
		
		if ( isset( $_COOKIE[ 'edgtf-like_' . $post->ID ] ) ) {
			$class = 'edgtf-like liked';
			$title = esc_html__( 'You already like this!', 'elaine' );
		}
...
if ( ! function_exists( 'elaine_edge_create_like' ) ) {
	function elaine_edge_create_like() {
		ElaineEdgeClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'elaine_edge_create_like' );
}