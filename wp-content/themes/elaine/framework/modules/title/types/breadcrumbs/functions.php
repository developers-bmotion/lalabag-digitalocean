...
if ( ! function_exists( 'elaine_edge_set_title_breadcrumbs_type_for_options' ) ) {
	/**
	 * This function set breadcrumbs title type value for title options map and meta boxes
	 */
	function elaine_edge_set_title_breadcrumbs_type_for_options( $type ) {
		$type['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'elaine' );
		
		return $type;
	}
	
	add_filter( 'elaine_edge_filter_title_type_global_option', 'elaine_edge_set_title_breadcrumbs_type_for_options' );
	add_filter( 'elaine_edge_filter_title_type_meta_boxes', 'elaine_edge_set_title_breadcrumbs_type_for_options' );
}

if ( ! function_exists( 'elaine_edge_custom_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs
	 */
	function elaine_edge_custom_breadcrumbs() {
		global $post;
		
		$pageid    = elaine_edge_get_page_id();
		$homeLink  = esc_url( home_url( '/' ) );
		$home      = esc_html__( 'Home', 'elaine' );
		...
		$output = '<div itemprop="breadcrumb" class="edgtf-breadcrumbs ' . esc_attr( $holder_classes ) . '" ' . elaine_edge_get_inline_attr( $holder_styles, 'style', ';' ) . '>';
			...
			if ( is_home() && ! is_front_page() ) {
				...
				if ( is_tag() ) {
					$childContent .= $before . esc_html__( 'Posts tagged ', 'elaine' ) . '"' . single_tag_title( '', false ) . '"' . $after;
					...
					$childContent    .= $before . esc_html__( 'Articles posted by ', 'elaine' ) . get_the_author_meta( 'display_name', $author_id ) . $after;
					...
					$childContent .= $before . esc_html__( 'Search results for ', 'elaine' ) . '"' . get_search_query() . '"' . $after;
					
				} elseif ( is_404() ) {
					$childContent .= $before . esc_html__( 'Error 404', 'elaine' ) . $after;
				...
				if ( get_query_var( 'paged' ) ) {
					$childContent .= $before . " (" . esc_html__( 'Page', 'elaine' ) . ' ' . get_query_var( 'paged' ) . ")" . $after;
					
				}
				
				$output .= apply_filters( 'elaine_edge_filter_breadcrumbs_title_child_output', $childContent, $delimiter, $before, $after );
			}
		
		$output .= '</div>';
		
		echo wp_kses( apply_filters( 'elaine_edge_filter_breadcrumbs_title_output', $output ), array(
			'div'  => array(
				'itemprop' => true,
				'id'       => true,
				'class'    => true,
