...
namespace ElaineEdgeNamespace\Modules\Header\Lib;

/**
 * Class ElaineEdgeClassHeaderConnector
 *
...
class ElaineEdgeClassHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'elaine_edge_action_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'elaine_edge_filter_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'elaine_edge_filter_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'elaine_edge_action_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to elaine_edge_filter_title_content_padding filter
	 *
	 * @return int
	 */
