...
if ( ! function_exists( 'elaine_edge_header_skin_class' ) ) {
	/**
	 * Function that adds header style class to body tag
	 */
	function elaine_edge_header_skin_class( $classes ) {
		$header_style     = elaine_edge_get_meta_field_intersect( 'header_style', elaine_edge_get_page_id() );
		$header_style_404 = elaine_edge_options()->getOptionValue( '404_header_style' );
	...
	add_filter( 'body_class', 'elaine_edge_header_skin_class' );
}

if ( ! function_exists( 'elaine_edge_sticky_header_behaviour_class' ) ) {
	/**
	 * Function that adds header behavior class to body tag
	 */
	function elaine_edge_sticky_header_behaviour_class( $classes ) {
		$header_behavior = elaine_edge_get_meta_field_intersect( 'header_behaviour', elaine_edge_get_page_id() );
	...
	add_filter( 'body_class', 'elaine_edge_sticky_header_behaviour_class' );
}

if ( ! function_exists( 'elaine_edge_menu_dropdown_appearance' ) ) {
	/**
	 ...
	 */
	function elaine_edge_menu_dropdown_appearance( $classes ) {
		$dropdown_menu_appearance = elaine_edge_options()->getOptionValue( 'menu_dropdown_appearance' );
	...
	add_filter( 'body_class', 'elaine_edge_menu_dropdown_appearance' );
}

if ( ! function_exists( 'elaine_edge_header_class' ) ) {
	/**
	 ...
	 */
	function elaine_edge_header_class( $classes ) {
		...
		$disable_menu_area_shadow = elaine_edge_get_meta_field_intersect( 'menu_area_shadow', $id ) == 'no';
		if ( $disable_menu_area_shadow ) {
			$classes[] = 'edgtf-menu-area-shadow-disable';
		}
		
		$disable_menu_area_grid_shadow = elaine_edge_get_meta_field_intersect( 'menu_area_in_grid_shadow', $id ) == 'no';
		if ( $disable_menu_area_grid_shadow ) {
			$classes[] = 'edgtf-menu-area-in-grid-shadow-disable';
		}
		
		$disable_menu_area_border = elaine_edge_get_meta_field_intersect( 'menu_area_border', $id ) == 'no';
		if ( $disable_menu_area_border ) {
			$classes[] = 'edgtf-menu-area-border-disable';
		}
		
		$disable_menu_area_grid_border = elaine_edge_get_meta_field_intersect( 'menu_area_in_grid_border', $id ) == 'no';
		if ( $disable_menu_area_grid_border ) {
			$classes[] = 'edgtf-menu-area-in-grid-border-disable';
		}
		
		if ( elaine_edge_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' &&
		     elaine_edge_get_meta_field_intersect( 'menu_area_grid_background_color', $id ) !== '' &&
		     elaine_edge_get_meta_field_intersect( 'menu_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'edgtf-header-menu-area-in-grid-padding';
		}
		
		$disable_logo_area_border = elaine_edge_get_meta_field_intersect( 'logo_area_border', $id ) == 'no';
		if ( $disable_logo_area_border ) {
			$classes[] = 'edgtf-logo-area-border-disable';
		}
		
		$disable_logo_area_grid_border = elaine_edge_get_meta_field_intersect( 'logo_area_in_grid_border', $id ) == 'no';
		if ( $disable_logo_area_grid_border ) {
			$classes[] = 'edgtf-logo-area-in-grid-border-disable';
		}
		
		if ( elaine_edge_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' &&
		     elaine_edge_get_meta_field_intersect( 'logo_area_grid_background_color', $id ) !== '' &&
		     elaine_edge_get_meta_field_intersect( 'logo_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'edgtf-header-logo-area-in-grid-padding';
		}
		
		$disable_shadow_vertical = elaine_edge_get_meta_field_intersect( 'vertical_header_shadow', $id ) == 'no';
		if ( $disable_shadow_vertical ) {
			$classes[] = 'edgtf-header-vertical-shadow-disable';
		}
		
		$disable_border_vertical = elaine_edge_get_meta_field_intersect( 'vertical_header_border', $id ) == 'no';
	...
	add_filter( 'body_class', 'elaine_edge_header_class' );
}

if ( ! function_exists( 'elaine_edge_header_area_style' ) ) {
	/**
	 * Function that return styles for header area
	 */
	function elaine_edge_header_area_style( $style ) {
		...
		$menu_area_background_color_rgba = elaine_edge_rgba_color( $menu_area_background_color, $menu_area_background_transparency );
		...
		if ( $menu_area_height !== '' ) {
			$menu_area_style['height'] = elaine_edge_filter_px( $menu_area_height ) . 'px !important';
		}
		...
		if ( $container_side_padding !== '' ) {
			...
			$current_style .= elaine_edge_dynamic_css( $menu_container_selector, $menu_container_styles );
		}
		...
		$menu_area_grid_background_color_rgba = elaine_edge_rgba_color( $menu_area_grid_background_color, $menu_area_grid_background_transparency );
		
		if ( ! empty( $menu_area_grid_background_color_rgba ) ) {
			$menu_area_grid_style['background-color'] = $menu_area_grid_background_color_rgba;
		}
		
		$current_style .= elaine_edge_dynamic_css( $menu_area_selector, $menu_area_style );
		$current_style .= elaine_edge_dynamic_css( $menu_area_grid_selector, $menu_area_grid_style );
		...
		if ( $dropdown_top_position !== '' ) {
			$dropdown_styles['top'] = elaine_edge_filter_suffix( $dropdown_top_position, '%' ) . '%';
		}
		
		$dropdown_selector = array( $class_prefix . ' .edgtf-page-header .edgtf-drop-down .second' );
		
		$current_style .= elaine_edge_dynamic_css( $dropdown_selector, $dropdown_styles );
		...
		$logo_area_background_color_rgba = elaine_edge_rgba_color( $logo_area_background_color, $logo_area_background_transparency );
		...
		if ( $logo_area_height !== '' ) {
			$logo_area_style['height'] = elaine_edge_filter_px( $logo_area_height ) . 'px !important';
		}
		...
		$logo_area_grid_background_color_rgba = elaine_edge_rgba_color( $logo_area_grid_background_color, $logo_area_grid_background_transparency );
		...
		if ( ! empty( $logo_area_style ) ) {
			$current_style .= elaine_edge_dynamic_css( $logo_area_selector, $logo_area_style );
		}
		
		if ( ! empty( $logo_area_grid_style ) ) {
			$current_style .= elaine_edge_dynamic_css( $logo_area_grid_selector, $logo_area_grid_style );
		}
		
		$current_style = apply_filters( 'elaine_edge_filter_add_header_page_custom_style', $current_style, $class_prefix, $page_id ) . $style;
		
		return $current_style;
	}
	
	add_filter( 'elaine_edge_filter_add_page_custom_style', 'elaine_edge_header_area_style' );
}