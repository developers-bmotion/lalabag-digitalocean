 ...
 */
if ( ! class_exists( 'ElaineEdgeClassSidebar' ) ) {
	class ElaineEdgeClassSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'elaine' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'edgtf-admin-sidebar-script', EDGE_ROOT . '/framework/admin/assets/js/edgtf-ui/edgtf-sidebar.js' );
			wp_enqueue_style( 'edgtf-admin-sidebar', EDGE_ROOT . '/framework/admin/assets/css/edgtf-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_attr__('Enter Name of the new Widget Area', 'elaine') . "' name='edgtf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_attr__('Add Widget Area', 'elaine') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function add_sidebar_area() {
			if ( ! empty( $_POST['edgtf-add-widget'] ) ) {
				...
				$name           = $this->get_name(sanitize_text_field($_POST['edgtf-add-widget']));
		...
		function delete_sidebar_area() {
			...
			if ( ! empty( $_POST['name'] ) ) {
				...
				if ( ( $key = array_search( $name, $this->sidebars ) ) !== false ) {
					...
					update_option( $this->stored, $this->sidebars );
					echo esc_html__("sidebar-deleted", "elaine");
				}
			}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'elaine_edge_filter_custom_widget_args', $args );
